...
- Two public entry points:
    - :meth:`AsyncCrawler.crawl`        — builds the Sitemap tree (for
      compatibility with the existing :mod:`usp.tree` API).
    - :meth:`AsyncCrawler.crawl_streaming` — yields raw URLs as they
      are discovered. **No pickle round-trip, no ``PagesXMLSitemap``
      materialisation** — O(1) memory in the number of pending sitemaps
      regardless of the total URL count.
"""
...
class AsyncCrawler:
    ...
    # ------------------------------------------------------------------
    # Public API: full tree (preserves the original contract)
    # ------------------------------------------------------------------
    async def crawl(self, root_urls: list[str]) -> AbstractSitemap:
        """Crawl from the given root URLs and return the synthetic root sitemap.

        Builds the full Sitemap object tree. For million-URL crawls prefer
        :meth:`crawl_streaming`.
        """
    ...
    # ------------------------------------------------------------------
    # Public API: streaming URLs (memory-bounded, O(1) in result size)
    # ------------------------------------------------------------------
    async def crawl_streaming(
        self,
        root_urls: list[str],
    ) -> AsyncIterator[str]:
        """Yield URLs as they are discovered.

        Memory usage is **O(concurrency)** in the number of in-flight
        sitemaps, **O(1)** in the number of total URLs. Safe for crawls
        of tens of millions of URLs.

        Yields raw URL strings; if you need the full SitemapPage metadata
        (priority, lastmod, images, news) fall back to :meth:`crawl`.
        """
        self._started_at = time.monotonic()
        norm_roots = [self._url_normalize(u) for u in root_urls]
    ...
    # ------------------------------------------------------------------
    # Internal: parser selection (3-tier fallback)
    # ------------------------------------------------------------------
    def _parse_one(
        self, final_url: str, content_bytes: bytes, parent_urls: set[str],
        level: int,
    ) -> tuple[AbstractSitemap, list[str] | None]:
        """Try lxml -> expat. Returns (sitemap, raw_child_urls).

        Note: the Rust path is handled separately in :meth:`_process_streaming`
        because it can stream raw URLs without constructing SitemapPage
        objects, so it does not need a Sitemap object at all.
        """
        kind = _sniff_root(content_bytes)
        sitemap: AbstractSitemap | None = None
        child_urls_raw: list[str] | None = None
        ...
        if sitemap is None:
            ...
            try:
                ...
                return InvalidSitemap(url=final_url, reason=f"parse: {ex}"), None
        ...
        if isinstance(sitemap, INDEX_TYPES) and child_urls_raw is None:
            child_urls_raw = []

        return sitemap, child_urls_raw

    # ------------------------------------------------------------------
    # Internal: streaming process (no PagesXMLSitemap pickle for Rust path)
    # ------------------------------------------------------------------
    async def _process_streaming(
        ...
        """Fetch + parse one URL, enqueue children, yield page URLs.

        For the Rust fast path, page URLs are **streamed as raw strings**
        (no ``SitemapPage`` construction, no tempfile pickle). For the
        lxml / expat fallbacks, we fall back to the upstream
        ``PagesXMLSitemap`` path (which uses a tempfile).
        """
        ...
        try:
            ...
            log.warning("fetch %s failed: %s", url, ex)
            self._sitemaps_failed += 1
            return

        if response.error is not None:
            log.warning("fetch %s error: %s", url, response.error)
            self._sitemaps_failed += 1
            return

        status = response.status_code()
        if not (200 <= status < 300):
            log.warning("fetch %s -> HTTP %d", url, status)
            self._sitemaps_failed += 1
            return

        final_url = self._url_normalize(response.url())

        # Redirect-loop detection: the response landed on a URL we
        # already saw (e.g. 301 -> parent). Without this check, a
        # misconfigured server could trap us in an infinite loop up to
        # ``max_depth``.
        if final_url in parent_urls:
            log.warning(
                "redirect loop: %s redirected to %s (already seen in parents); skipping",
                url, final_url,
            )
            self._sitemaps_failed += 1
            return
        ...
        # Detect if this came through the Rust fast path (we want to
        # stream raw URLs without SitemapPage overhead). We sniff first.
        kind = _sniff_root(content_bytes)
        ...
        # Fallback: build the Sitemap object, walk it to yield pages.
        sitemap, child_urls_raw = self._parse_one(
            final_url, content_bytes, new_parents, level
        )
        ...
        if isinstance(sitemap, INDEX_TYPES):
            ...
            try:
                ...
                log.warning("read pages %s: %s", final_url, ex)

    def _enqueue_children(
        self,
        child_urls_raw: list[str],
        final_url: str,
        new_parents: set[str],
        level: int,
        queue: "asyncio.Queue",
    ...
    # ------------------------------------------------------------------
    # Internal: legacy tree-building path (kept for `crawl()`)
    # ------------------------------------------------------------------
    async def _process(
        self,
        url: str,
        ...
        if final_url in parent_urls:
            ...
            self._results[url] = InvalidSitemap(
                url=url, reason=f"redirect loop to {final_url}"
            )
            return
